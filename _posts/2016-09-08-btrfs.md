...
- btrfs لینوکس      
- فایل سیستم btrfs  
description: فایل سیستم Btrfs      
---

Btrfs یا B-tree Filesystem یک فایل سیستم متن باز مبتنی بر مجوز GPL است که توسط شرکت هایی چون Oracle, Redhat, Fujitsu, Intel, Facebook, Linux Foundantion, Suse و … توسعه داده می شود.

این فایل سیستم دیسک های با ظرفیت حداکثر ۱۶ اگزابایت و فایل‌های با اندازه حداکثر ۸ اگزابایت را پشتیبانی می کند.

برای نامگذاری فایل‌های ایجاد شده بر روی این فایل سیستم تقریباً می‌توان از هر کاراکتری به جز '/' و NULL استفاده نمود.

برخی ‌از ویژگی‌های مهم Btrfs عبارتند از:

*   یکپارچه سازی دیسک (Defragmention) بصورت انلاین
*   تغییر اندازه پارتیشن ها (Volume) بصورت انلاین
*   حدف یا اضافه نمودن دیسک های سخت بصورت انلاین
*   بررسی فایل سیستم (Fsck) بصورت آفلاین
*   بررسی فایل‌ها و برطرف نمودن خودکار مشکلات آن‌ها بصورت انلاین (Data Scrubbing)
*   پشتیبانی از RAID0 , RAID1, RAID10
*   پشتیبانی از Subvolume ها
*   فشرده سازی (Transparent Compression)
*   بررسی Checksum بر روی داده‌ها و Metadata

و …

Btrfs در اکثر توزیع‌های معروف بصورت پیشفرض پشتیبانی می‌شود و برای استفاده از آن فقط می بایست ابزارهای مربوط به آن را در توزیع خود نصب کنیم.
...
```
یکی از خصوصیات جالب Btrfs این است که شما می‌توانید فایل سیستم رو بر روی یک یا چند دیسک خام و پارتیشن بندی نشده ایجاد کنید. برای مثال نیازی به استفاده از ابزارهایی همچون fdisk برای پارتیشن بندی نیست.
...
```
*   **نکته) در صورتی که دستور mkfs.btrfs رو بدون هیچ سوییچ و همراه با چند دیسک بکار ببریم بصورت پیشفرض از RAID0 (Stripe)برای ذخیره سازی داده‌ها و از (Mirror) RAID1 برای ذخیره سازی Metadata استفاده میشود. مزییت این روش در این است که علاوه بر بهبود سرعت خواندن و نوشتن داده‌ها در صورتی که یکی از دیسکها دچار مشکل شود به کمک Metadata می‌توانیم داده‌های از دست رفته را بازسازی کنیم.**

اگر مایل هستید که btrfs رو تنها بر روی یک دیسک ایجاد کنید و یا مایل نیستید که Data و Metadata بصورت RAID ذخیره سازی شود می‌توانید از سوییچ m- بصورت زیر استفاده نمایید:
```sh
mkfs.btrfs -m single /dev/sdb
```
همچنین در صورتی که بخواهید همین کار را با چندین دیسک انجام دهید نیز می‌توانید بصورت زیر عمل نمایید:
```sh
...
```
که در مثال فوق توسط سوییچهای m- و d- نحوه ذخیره سازی مشخص گردیده است.
...
```
یکی از ویژگی‌های Btrfs قابلیت فشرده‌سازی داده‌ها می‌باشد که موجب می‌شود داده‌ها بصورت فشرده شده بر روی دیسک ذخیره شوند که موجب صرفه جویی در مصرف فضای دیسک می گردد. Btrfs از دو متد فشرده‌سازی zlib و lzo برای فشرده‌سازی استفاده می کند.

*   **نکته) Btrfs بصورت پیشفرض از zlib برای فشرده‌سازی استفاده می کند.**
...
از دیگر ویژگی‌های Btrfs امکان تغییر حجم Volume ها بصورت انلاین می باشد. بعنوان مثال دستور زیر مقدار 2G از فضای فایل سیستم mnt/ می کاهد.
```sh
...
```
برای اضافه نمودن دیسک جدید به فایل سیستم مورد نظرمان بصورت زیر عمل می کنیم:
```sh
...
```
در صورتی که بخواهیم RAID Level را برای فایل سیستم ایجاد شده تغییر دهیم می‌توانیم بصورت زیر عمل کنیم:
```sh
btrfs balance start -dconvert=raid1 -mconvert=raid1 /mnt
```
یکی دیگر از قابلیت‌های جالب در btrfs مفهومی به نام Subvolume است. در btrfs می‌توانیم در هر فایل سیستم Subvolume هایی ایجاد کنیم و سپس از این Subvolume ها Snapshot بگیریم و یا آن‌ها را بصورت مستقل از فایل سیستم اصلی Mount کنیم.
...
```
برای Mount نمودن یک Subvolume بصورت زیر عمل می کنیم:

ابتدا می بایست فایل سیستم اصلی را در صورتی که Mount می‌باشد Unmount کنیم:
```sh
umount /mnt
```
و سپس توسط دستور زیر و به وسیله Subvolume ID انرا بصورت زیر Mount می کنیم:
```sh
mount -o subvolid=272 /dev/sdb /mnt
```
که در اینجا Subvolume ID مورد نظرما 266 می باشد. همچنین می‌توانیم بجای استفاده از ID از نام Subvolume استفاده کنیم:
```sh
mount -o subvol=sv1 /dev/sdb /mnt
```
*   **نکته)‌ ID مربوط به Volume ریشه همواره 0 می باشد.**

برای تغییر Default Volume در زمان Mount می‌توانیم بصورت زیر عمل کنیم:
```sh
btrfs subvolume set-default 266 /mnt
```
از این پس هنگام Mount نمودن فایل سیستم, بجای Volume ریشه Subvolume مشخص شده بصورت پیشفرض Mount می گردد.
...
یکی دیگر از امکانات مفید btrfs امکان تهیه Snapshot از Subvolume ها بصورت انلاین می باشد. توسط Snapshot ها می‌توانیم از Data های ذخیره شده در Subvolume ها کپی پشتیبان تهیه کنیم تا در مواقع لزوم Data ها را به حالت اولیه بازگردانیم.
